...
def nfs3_hs_tga_converter(inputFile, convType, flipToggle, editToggle, tgaOrigin):
    ...
    if convType == 0:
        pdb.gimp_context_set_sample_threshold_int(35)
        pdb.gimp_image_select_color(inputImage, 2, imageMask, (96,96,96))
        if (pdb.gimp_selection_is_empty(inputImage) == 0):
            pdb.gimp_selection_sharpen(inputImage)
            pdb.gimp_context_set_foreground((224,224,224))
            pdb.gimp_edit_fill(imageMask, 0)
        pdb.gimp_image_select_color(inputImage, 2, imageMask, (160,160,160))
        if (pdb.gimp_selection_is_empty(inputImage) == 0):
            pdb.gimp_selection_sharpen(inputImage)
            pdb.gimp_context_set_foreground((96,96,96))
            pdb.gimp_edit_fill(imageMask, 0)
    ...
    if convType == 1:
        ...
        pdb.gimp_image_select_color(inputImage, 2, imageMask, (32,32,32))
        if (pdb.gimp_selection_is_empty(inputImage) == 0):
            pdb.gimp_selection_sharpen(inputImage)
        ...
        if (pdb.gimp_selection_is_empty(inputImage) == 0):
            pdb.gimp_selection_sharpen(inputImage)
            pdb.gimp_context_set_foreground((64,64,64))
            pdb.gimp_edit_fill(imageMask, 0)
        pdb.gimp_image_select_color(inputImage, 2, imageMask, (224,224,224))
        if (pdb.gimp_selection_is_empty(inputImage) == 0):
            pdb.gimp_selection_sharpen(inputImage)
            pdb.gimp_context_set_foreground((100,100,100))
            pdb.gimp_edit_fill(imageMask, 0)
        pdb.gimp_image_select_color(inputImage, 2, imageMask, (64,64,64))
        if (pdb.gimp_selection_is_empty(inputImage) == 0):
            pdb.gimp_selection_sharpen(inputImage)
...
register(
    ...
    "", # image format this plug-in works in (*, RGB, RGB*, RGBA, GRAY etc...)
    [
        ...
        (PF_OPTION, "tgaOrigin", "Exported TGA origin: ", 1,
             (
